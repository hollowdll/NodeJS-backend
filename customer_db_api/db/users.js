...
ALTER TABLE IF EXISTS public.users
    ...
    db.query(sql, (err, result) => {
        if (err) {
            ...
            console.log("Successfully created table 'users': ", result);
        }
    });
}
...
const createAdminUser = () => {
    ...
    bcrypt.hash(
        process.env.USERS_TABLE_ADMIN_PASSWORD,
        saltRounds
    )
    .then(hash => {
        const query = {
            ...
            values: [
                process.env.USERS_TABLE_ADMIN_EMAIL,
                hash,
            ]
        }
        ...
        db.query(query, (err, _result) => {
            if (err) {
                return console.error('Error inserting admin user into users table', err.stack)
            } else {
